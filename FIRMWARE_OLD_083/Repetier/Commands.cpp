...
void process_command(GCode *com,byte bufferedCommand)
{
  ...
  center of arc when h_x2_div_d is positive ->  x <----- | -----> x <- center of arc when h_x2_div_d is negative
    ...
    switch( com->M ) {
      ...
      case 115: {// M115
#if DRIVE_SYSTEM==3
        out.println_P(PSTR("FIRMWARE_NAME:Repetier-ORION " REPETIER_VERSION " FIRMWARE_URL:https://github.com/seemecnc/ORION PROTOCOL_VERSION:1.0 MACHINE_TYPE:ORION EXTRUDER_COUNT:" XSTR(NUM_EXTRUDER) " REPETIER_PROTOCOL:2"));
#else
#if DRIVE_SYSTEM==0
